...
module Beseder.BlackJack.Resource.BlackJackRes where

import           Haskus.Utils.Variant
import           Protolude
import           Beseder.Base.Common                                               
import           Beseder.Resources.State.PureRes
import           Beseder.BlackJack.Game.BlackJack
...
data StartBlackJackGame = StartBlackJackGame deriving (Eq, Show, Typeable)
data PlayerHit = PlayerHit deriving (Eq, Show, Typeable)
data PlayerStand = PlayerStand deriving (Eq, Show, Typeable)
data DealerFirstStep = DealerFirstStep deriving (Eq, Show, Typeable)
data DealerNextStep = DealerNextStep deriving (Eq, Show, Typeable)

instance GetInstance PlayerHit where getInstance = PlayerHit
instance GetInstance PlayerStand where getInstance = PlayerStand
...
cardsOfPlayer :: St (PSt (GameState gameStatus playerStatus handStatus)) name -> HandCards
cardsOfPlayer st = cardsOf getPlayerCards st

cardsOfDealer :: St (PSt (GameState gameStatus playerStatus handStatus)) name  -> HandCards
