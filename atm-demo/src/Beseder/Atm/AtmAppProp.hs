...
module Beseder.Atm.AtmAppProp where
...
atmAppAsrtData :: forall m resAcc. AccountRes.ResPar m resAcc -> STransData m NoSplitter _ ()
...
handleAuthentication :: STransData m sp _ () --IsActiveUser _ ()
handleAuthentication = do --repeatUnless/ repeatWhile
  ...
  repeatBreakingOn @("acc" :? IsAccountBlocked 
    ...
    pcode <- prop @PassCodeProp #term 
    cardInfo <- prop @CardDetailsProp #card 
...
handleBalance :: 
  -- forall resAcc m sp. Account m resAcc => 
  STransData m sp _ () --IsActiveUser _ ()
handleBalance = do
...
handleWithdrawal :: -- forall resTerm m sp. (Terminal m resTerm) => 
  STransData m sp _ () 
handleWithdrawal = do
  assert @("term" :? IsWithdrawalSelected :&& "acc" :? IsUserAuthenticated)
  amnt <- prop @WithdrawalAmountProp #term
